...
| production_ready | Applies production-safe defaults: enables `always_on` for Premium/Dedicated plans, enforces `https_only`, and sets `max_scale_out_limit` to 100 for Consumption plans. Note: For high-traffic production workloads, you may want to adjust or remove the scale limit. |
...
The `production_ready` keyword applies production-safe defaults with a single line:
...
let productionFunction = functions {
    ...
    production_ready  // Enables AlwaysOn, HTTPS, and scale limits
}
...
let consumptionFunction = functions {
    ...
    production_ready  // Sets max_scale_out_limit to 100, enforces HTTPS
}
