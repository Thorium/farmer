...
Use the `production_ready` keyword to apply all production defaults:
...
let productionFunction = functions {
    ...
    production_ready  // ← Applies all production defaults
}
```

**What `production_ready` does:**
...
| **HTTPS Only** | Always | `https_only` or `production_ready` | Security best practice |
...
| **AlwaysOn** | Premium/Dedicated | `always_on` or `production_ready` | Prevent cold starts |
...
The `production_ready` keyword sets `max_scale_out_limit` to 100 for Consumption plans. This is a **sensible default for development and testing** to prevent unexpected costs.
...
2. **Production helpers** - `production_ready` keyword applies best practices as a starting point
...
Start with `production_ready` and customize for your production requirements!
